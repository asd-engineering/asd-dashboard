import { test, expect } from './fixtures'

declare global {
  interface Window { evicted: string[] }
}

test.describe('Eviction Modal v2', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('domcontentloaded')
    await page.waitForFunction(() => document.body.dataset.ready === 'true')
  })

  test('undefined-requiredCount: auto-select LRU selects exactly 1 and navigates immediately', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: 'A', serviceName: 'SvcA', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 },
        { id: 'b', title: 'B', serviceName: 'SvcB', icon: '🧱', boardIndex: 0, viewIndex: 1, lruRank: 1 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: null,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()
    await expect(modal.locator('#eviction-header')).toHaveText('1 widget must be removed to continue navigation.')
    await expect(modal.locator('#eviction-counter')).toHaveText('0 of 1 widgets selected')
    await modal.locator('#evict-lru-btn').click()
    await expect(modal).toBeHidden()
    const evicted = await page.evaluate(() => window.evicted)
    expect(evicted).toEqual(['a'])
  })

  test('defined-requiredCount: manual selection up to N enables Continue and evicts', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const mk = (id, title, serviceName, boardIndex, viewIndex, lruRank) => ({ id, title, serviceName, icon: '🧱', boardIndex, viewIndex, lruRank })
      const items = [
        mk('a', null, 'Alpha', 0, 0, 0),
        mk('b', 'Bravo', 'Beta', 0, 1, 1),
        mk('c', 'Charlie', 'Gamma', 1, 0, 2)
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 2,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()
    await expect(modal.locator('#eviction-header')).toHaveText('2 widgets must be removed to continue navigation.')
    const list = modal.locator('#eviction-list')
    await expect(list).toHaveCSS('flex-direction', 'column')
    const listItems = list.locator('label')
    await expect(listItems).toHaveCount(3)
    await expect(listItems.nth(0)).toContainText('Alpha — Board 1, View 1')
    await expect(listItems.nth(1)).toContainText('Bravo — Board 1, View 2')
    await expect(modal.locator('#eviction-disclaimer')).toBeVisible()
    await expect(modal.locator('input[type="checkbox"][role="switch"]')).toHaveCount(0)
    const counter = modal.locator('#eviction-counter')
    await expect(counter).toHaveText('0 of 2 widgets selected')
    const boxes = modal.locator('#eviction-list input[type="checkbox"]')
    await expect(boxes).toHaveCount(3)
    await boxes.nth(0).check()
    await expect(counter).toHaveText('1 of 2 widgets selected')
    const continueBtn = modal.locator('button:has-text("Continue")')
    await expect(continueBtn).toBeDisabled()
    await boxes.nth(1).check()
    await expect(counter).toHaveText('2 of 2 widgets selected')
    await expect(continueBtn).toBeEnabled()
    await continueBtn.click()
    await expect(modal).toBeHidden()
    const evicted = await page.evaluate(() => window.evicted)
    expect(evicted).toEqual(['a', 'b'])
  })

  test('LRU flow: auto-select removes correct N by rank', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: 'A', serviceName: 'SvcA', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 },
        { id: 'b', title: 'B', serviceName: 'SvcB', icon: '🧱', boardIndex: 0, viewIndex: 1, lruRank: 1 },
        { id: 'c', title: 'C', serviceName: 'SvcC', icon: '🧱', boardIndex: 1, viewIndex: 0, lruRank: 2 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 2,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()
    await modal.locator('#evict-lru-btn').click()
    await expect(modal).toBeHidden()
    const evicted = await page.evaluate(() => window.evicted)
    expect(evicted).toEqual(['a', 'b'])
  })

  test('Cancel leaves state unchanged', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: 'A', serviceName: 'SvcA', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 1,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()
    await modal.locator('button:has-text("Cancel")').click()
    await expect(modal).toBeHidden()
    const evicted = await page.evaluate(() => window.evicted)
    expect(evicted).toEqual([])
  })

  test('Selection limit: checkboxes disabled when limit reached', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: 'A', serviceName: 'SvcA', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 },
        { id: 'b', title: 'B', serviceName: 'SvcB', icon: '🧱', boardIndex: 0, viewIndex: 1, lruRank: 1 },
        { id: 'c', title: 'C', serviceName: 'SvcC', icon: '🧱', boardIndex: 1, viewIndex: 0, lruRank: 2 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 2,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    const boxes = modal.locator('#eviction-list input[type="checkbox"]')
    const counter = modal.locator('#eviction-counter')

    // All checkboxes should be enabled initially
    await expect(boxes.nth(0)).toBeEnabled()
    await expect(boxes.nth(1)).toBeEnabled()
    await expect(boxes.nth(2)).toBeEnabled()

    // Select first two (a, b)
    await boxes.nth(0).check()
    await boxes.nth(1).check()
    await expect(counter).toHaveText('2 of 2 widgets selected')
    await expect(boxes.nth(0)).toBeChecked()
    await expect(boxes.nth(1)).toBeChecked()

    // Third checkbox should be disabled when limit reached
    await expect(boxes.nth(2)).toBeDisabled()

    // Unchecking one re-enables the third
    await boxes.nth(0).uncheck()
    await expect(boxes.nth(2)).toBeEnabled()

    // Select different combination and continue
    await boxes.nth(2).check()
    await expect(counter).toHaveText('2 of 2 widgets selected')

    const continueBtn = modal.locator('button:has-text("Continue")')
    await continueBtn.click()
    await expect(modal).toBeHidden()
    const evicted = await page.evaluate(() => window.evicted)
    expect(evicted.sort()).toEqual(['b', 'c'].sort())
  })

  test('Empty items: shows modal with disabled buttons', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 1,
        items: [],
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()
    await expect(modal.locator('#eviction-list label')).toHaveCount(0)
    await expect(modal.locator('#evict-lru-btn')).toBeDisabled()
    // Continue button should be hidden when no items
    await expect(modal.locator('button:has-text("Continue")')).toBeHidden()
    // Cancel should still work
    await modal.locator('button:has-text("Cancel")').click()
    await expect(modal).toBeHidden()
  })

  test('Keyboard navigation: Tab cycles within modal', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: 'A', serviceName: 'SvcA', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 1,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()

    // First focusable element should be focused (the checkbox)
    const checkbox = modal.locator('#eviction-list input[type="checkbox"]')
    await expect(checkbox).toBeFocused()

    // Select the checkbox to enable the Continue button
    await checkbox.check()

    // Now Tab through: Continue (enabled) -> Auto-Remove -> Cancel
    await page.keyboard.press('Tab')
    await expect(modal.locator('button:has-text("Continue")')).toBeFocused()
    await page.keyboard.press('Tab')
    await expect(modal.locator('#evict-lru-btn')).toBeFocused()
    await page.keyboard.press('Tab')
    await expect(modal.locator('button:has-text("Cancel")')).toBeFocused()

    // Tab from last should wrap to first (checkbox)
    await page.keyboard.press('Tab')
    await expect(checkbox).toBeFocused()

    // Shift+Tab from first should wrap to last (Cancel)
    await page.keyboard.press('Shift+Tab')
    await expect(modal.locator('button:has-text("Cancel")')).toBeFocused()

    // Close with Cancel using Enter
    await page.keyboard.press('Enter')
    await expect(modal).toBeHidden()
  })

  test('Display name: null title falls back to serviceName', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: null, serviceName: 'MyServiceName', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 1,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    await expect(modal).toBeVisible()
    const label = modal.locator('#eviction-list label').first()
    // Should display serviceName when title is null
    await expect(label).toContainText('MyServiceName — Board 1, View 1')
  })

  test('Unchecking a selected item re-enables other checkboxes', async ({ page }) => {
    await page.evaluate(async () => {
      const { openEvictionModal } = await import('/component/modal/evictionModal.js')
      const items = [
        { id: 'a', title: 'A', serviceName: 'SvcA', icon: '🧱', boardIndex: 0, viewIndex: 0, lruRank: 0 },
        { id: 'b', title: 'B', serviceName: 'SvcB', icon: '🧱', boardIndex: 0, viewIndex: 1, lruRank: 1 }
      ]
      window.evicted = []
      openEvictionModal({
        reason: 'test',
        maxPerService: 5,
        requiredCount: 1,
        items,
        onEvict: async ids => { window.evicted = ids }
      })
    })
    const modal = page.locator('#eviction-modal')
    const boxes = modal.locator('#eviction-list input[type="checkbox"]')
    const counter = modal.locator('#eviction-counter')

    // Select first
    await boxes.nth(0).check()
    await expect(counter).toHaveText('1 of 1 widgets selected')
    // Second checkbox should be disabled when limit reached
    await expect(boxes.nth(1)).toBeDisabled()

    // Uncheck first
    await boxes.nth(0).uncheck()
    await expect(counter).toHaveText('0 of 1 widgets selected')
    // Both checkboxes should now be enabled
    await expect(boxes.nth(0)).toBeEnabled()
    await expect(boxes.nth(1)).toBeEnabled()
  })
})
