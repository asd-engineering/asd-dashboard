...
test.describe("Dashboard Config - Base64 via URL Params", () => {
  test("loads dashboard from valid config_base64 and services_base64", async ({
    page,
  }) => {
...
test.describe("Dashboard Config - Remote via URL Params", () => {
  test("loads dashboard from valid config_url and services_url", async ({
    page,
  }) => {
...
test.describe("Dashboard Config - LocalStorage Behavior", () => {
  test("after first load, config is used from localStorage", async ({
    page,
  }) => {
  ...
  test("changes via modal are saved and persist", async ({ page }) => {
    ...
    await page.fill(
      "#config-json",
      JSON.stringify({ ...ciConfig, boards: [] }),
    );
    await page.click("#config-modal .modal__btn--save");
    await page.reload();
    // await waitForDashboardReady(page)
  ...
  test("removing config from localStorage shows popup again", async ({
    page,
  }) => {
    await page.goto(`/?config_base64=${b64(ciConfig)}`);
    // await waitForDashboardReady(page)
...
test.describe("Dashboard Functionality - Building from Services", () => {
  test("user can add board, view, and widget from services", async ({
    page,
  }) => {
    const cfg = {
      ...ciConfig,
...
test.describe("Dashboard Config - Priority and Overwriting", () => {
  test("base64 param overrides existing localStorage config", async ({
    page,
  }) => {
    await page.goto(
  ...
  test("without new param, existing config remains active", async ({
    page,
  }) => {
    await page.goto(
      `/?config_base64=${b64({ ...ciConfig, globalSettings: { theme: "dark" } })}`,
